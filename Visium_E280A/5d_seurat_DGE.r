...
dDir <- "/home/eger/projects/Brain_Visium/v3/DGE_d/"
...
################################################################################
### Merge - add METADATA ###
seurat_obj <- merge(C140_c12, y = c(C363_c12, C364_c12, 
                C364_c1),
...
for (reg in c("WM", "GM")){
    ...
    if (file.exists(paste0(dDir, "DGE_", reg, "_E280A_v_CN_4_samples.csv"))){
        fname <- paste0(dDir, "DGE_", reg, "_E280A_v_CN_4_samples.csv") 
        df2 <- read.csv(fname, )[, -1] 
    } else {
        ...
        # for volcano plots
        fname <- paste0(dDir, "DGE_all_", reg, "_E280A_v_CN_4_samples.csv") 
